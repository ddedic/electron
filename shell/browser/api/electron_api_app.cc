...
// from 1) related web sets and 2) from an app that has third-party cookies blocked
// to use the requestStoreAccessFor API to grant the same permissions for both (more than 2?)
// of the provided sites of the same origin. If it merges with this comment in place,
...
if (electron::fuses::IsSameSiteOriginCookiesEnabled()) {
  ...
  // void requestStorageAccessFor(std::string raw_sets) {
    ...
    if (AppBlocksThirdPartyCookies()) {
      ModifyCookieSameSite(url, name, "None");
    }
    ElectronBrowserClient::Get()->RequestStorageAccessFor(requestedOrigin);
  }
}
